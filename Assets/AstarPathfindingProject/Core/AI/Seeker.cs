...
public class Seeker : MonoBehaviour, ISerializationCallbackReceiver {
	...
	void Start() {
		if(transform != null && transform.position.z != 0) {
			UnityEngine.Debug.LogWarning("AStarPathfinding doesn't like it when z != 0 on an agent's transform. This can cause strange pathfinding behaviour.");
		}
	}
