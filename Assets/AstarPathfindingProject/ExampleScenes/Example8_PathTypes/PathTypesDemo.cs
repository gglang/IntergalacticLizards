...
public class PathTypesDemo : MonoBehaviour {
	...
	public void OnPathComplete (Path p) {
		...
		if (p.GetType () == typeof (MultiTargetPath)) {
			...
			/*List<Vector3> pts = Pathfinding.Path  Utilities.Instance.GetPointsOnNodes (nodes, 20, 0);
			...
			Pathfinding.Path  Utilities.Instance.GetPointsAroundPoint (start.position, AstarPath.active.astarData.graphs[0] as IRaycastableGraph, pts, 0, 1);
