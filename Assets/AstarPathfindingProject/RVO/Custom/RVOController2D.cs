...
namespace Pathfinding.RVO {
	 ...
	 * \astarpro
	 */
	[AddComponentMenu("Pathfinding/Local Avoidance/RVO Controller 2D")]
	public class RVOController2D : MonoBehaviour {
		...
		public void OnEnable () {
			...
			if (rvoAgent != null) {
				...
				rvoAgent = simulator.AddAgent (XZPosition);
			}

			UpdateAgentProperties ();
			rvoAgent.Teleport (XZPosition);
		}

		Vector3 XZPosition{
			get { return new Vector3(transform.position.x, transform.position.z, transform.position.y);}	// FIXME this is a weird hack due to our unconventional use of dimensions
		}
		...
		public void Update () {
			...
			rvoAgent.DesiredVelocity = new Vector3(desiredVelocity.x, 0, desiredVelocity.y) + force*wallAvoidForce;

			tr.position = new Vector3(position.x, position.z, position.y) - center;	// FIXME this is a weird hack due to our unconventional use of dimensions
